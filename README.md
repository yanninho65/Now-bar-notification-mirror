...
- **`/sport`** (`SportWatchSync`, per match also `canMute` = the notification has Sofascore's "Mettre l'événement en silencieux" action (`muteActionOf`: semantic MUTE or label silenc/mute/sourdine), coalesced 400 ms on the listener's main thread, deduped by signature, images cached per posting): `matches` (every active Sofascore notif, max 20: key, postTimeMillis, title + putMatch fields, asset `img_<i>`), `followedKey` (CHOSEN and still active), `apps` (SportAppsPrefs with counts, same format as `/messages`), `icon_<pkg>`. Triggered by every `refresh()`, by posts/removals of a sport app, and `requestSportSync()` (app selection change, import).
- Watch → phone `/sportdetail/{follow,dismiss,mute,open}` (JSON `key`; follow with blank key = back to auto) → `SofascoreNotificationListenerService.followFromWatch` / `dismissFromWatch` (same path as the widget dismiss) / `muteFromWatch` (fires the mute action, then `dismiss` like the delete button) / `openOnPhoneFromWatch` (`postOpenOnPhone`, contentIntent else Sofascore launch; doesn't dismiss).
