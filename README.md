...
- Tapping a Sport match or a Toutes-notifs tile no longer opens the source app directly — it shows that ONE notification full-format, reusing the exact same `widget_latest_content` block the Dernière notif view renders with (title, text, image, dismiss button, up to three action buttons if "Actions dans le widget" is on), and tapping the text still opens the original notification/app exactly like the real Dernière notif view already does. This never touches which notification the real Dernière notif view itself shows — "je précise bien que ça ne change rien à la vue dernière notif [...] elle doit toujours bien montrer la dernière notif" — peeking is purely a different way to render a tapped tile, layered on top, not a change to LATEST's own selection logic (`WidgetNotificationStore` is untouched by it).
- While peeking, the left column swaps to the rotating-arrows glyph as a "back to the tile grid" button (Yann: "Mettre les flèches tournantes pour le symboliser") — both the app-icon slot and the small toggle underneath it are bound to the same close action, same enlarged-tap-zone reasoning as the left toggle below.
...
**Right button, context-dependent icon (NEW 18/09/2026)** — Yann: "En vue sport, sur la droite, remplacer les flèches par un bouton cloche et flèches qui tourne [...] En vue toutes notifs, mettre l'icone blanche de Sofascore avec flèches aussi". Same toggle action as before (Sport ↔ Toutes notifs), only the icon changes:
- On **Sport**, a static bell-badged arrows glyph (`ic_widget_switch_to_notifs` — a plain drawable, since both the arrows and the generic bell are this app's own icons), hinting at the Toutes-notifs view it leads to.
- On **Toutes notifs**, the same arrows glyph badged instead with a white silhouette derived, at render time, from Sofascore's own currently-installed app icon (`sofascoreToggleIconBitmap`) — the same general technique Android itself uses to derive a themed/monochrome status-bar icon from an app's adaptive-icon layer — rather than a hand-drawn recreation of Sofascore's actual logo. Falls back to the plain arrows glyph if Sofascore isn't installed or the composite fails for any reason.
