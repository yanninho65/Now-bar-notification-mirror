...
- **`WatchNotificationSync`** (top-level) — mirrors the widget's "Dernière notif" slot to the watch's "Notification" complication, called from the same `NowBarWidgetProvider.pushLive()` choke point that feeds `WidgetNotificationStore`, so the two never drift independently.
...
├── WatchNotificationSync.kt           sends "Dernière notif" to the watch's "Notification" complication (Wear Data Layer API)
...
├── NotificationDataCodec.kt           shared decode of the /notification DataMap
