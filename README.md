...
Sofascore (`com.sofascore.results`) stays in this list like any other app, and selecting a mode for it here is still what makes it appear in the real Now Bar system notification and in the **Dernière notif** widget view (this listener is the only thing that builds those) — see the "Sofascore no longer races itself here" bullet below for the narrower fix (18/09/2026) that stops it from ALSO duplicating into "Toutes notifs".
...
- **Toutes notifs** — up to 5 recently received notifications, same tile layout as Sport, fed from two sources into one shared history (`WidgetAllNotificationsStore`): every notification the Accueil tab mirrors (any app, ALL or LATEST mode, always their ORIGINAL title — see the "Titre ↔ texte" fix above), and every Sofascore notification (reusing the exact same parsing as the Sport view, so a Sofascore entry here needs no separate mirroring setup for `com.sofascore.results` — the Sport tab's own listener feeds it directly; see the "Sofascore no longer races itself here" bullet below for why the Accueil listener's own copy of the same notification is deliberately skipped here). Per tile:
  ...
  - **Sofascore no longer races itself here (fixed 18/09/2026, corrected same day)** — Yann: "j'ai encore des applis Sofascore qui s'affichent bien en vue sport mais s'affichent comme les autres notifs en vue toutes notifs. C'est aléatoire et parfois j'ai même deux icônes pour un même match." Cause: if Sofascore's package was ALSO selected in the Accueil app list (a plain "Dernière notif"/"Toutes" app), `MirrorNotificationListener` pushed its OWN generic (image+title) tile for the exact same notification the Sport tab's listener was independently pushing as a match tile into this same shared history — whichever push landed last decided the tile's presentation, and when their identities didn't line up exactly, both ended up coexisting as two tiles for one match. A first version of this fix went too far and made `MirrorNotificationListener` ignore Sofascore's package everywhere, including for the real Now Bar notification and the Dernière notif widget view, which broke Sofascore mirroring there entirely (Yann: "Ah mais Sofascore n'apparaît plus dans la now bar, il y a un problème"). The fix is now narrower: Sofascore stays selectable in the Accueil app list and `MirrorNotificationListener` still mirrors it normally everywhere — the real Now Bar notification, the Dernière notif view, all of it — except for exactly one push, its OWN copy of the generic tile into this shared "Toutes notifs" history, which is now skipped (`pushAllNotifsHistoryOnly` and `mirror()` both special-case `com.sofascore.results` for that one call only). A Sofascore entry here can therefore only ever come from the Sport tab's own listener, so it always renders as a match tile, never a generic one, and never twice — while the real Now Bar notification and Dernière notif view are unaffected and keep working exactly as before.
  ...
  - **Icons no longer sink on a short tile (fixed 18/09/2026)** — Yann: "toujours aligner les icônes. Les aligner comme s'il y avait toujours 2 lignes en dessous. Là, quand il y en a une seule ligne, l'image descend, elle ne devrait pas." Both this row and the Sport row used `android:gravity="center_vertical"`, so a tile whose own content was shorter than its tallest sibling — chiefly a generic tile whose title wraps to only 1 line instead of 2 — got vertically centered within the row's full height, splitting the difference evenly above and below it and pushing its image down relative to any neighbour with more text underneath. Both rows now use `android:gravity="top"` instead, so every tile's image stays flush with the top of the row no matter how many lines its own text ends up using.
...
**Tapping a tile now "peeks" it full-format instead of leaving the widget (NEW 18/09/2026)** — Yann: "En vue toutes notifs ou sport, cliquer sur une icône doit ouvrir le texte de la notification en question sous le même format que la dernière notif affichée". A fourth, cross-cutting state (`WidgetPeekPrefs`) sits on top of the three views above:

- Tapping a Sport match or a Toutes-notifs tile no longer opens the source app directly — it shows that ONE notification full-format, reusing the exact same `widget_latest_content` block the Dernière notif view renders with (title, text, image, dismiss button, up to three action buttons if "Actions dans le widget" is on). This never touches which notification the real Dernière notif view itself shows — "je précise bien que ça ne change rien à la vue dernière notif [...] elle doit toujours bien montrer la dernière notif" — peeking is purely a different way to render a tapped tile, layered on top, not a change to LATEST's own selection logic (`WidgetNotificationStore` is untouched by it).
- **Tapping the text now also closes the peek (FIXED 18/09/2026)** — Yann: "quand je clique sur une icône puis sur le texte pour ouvrir la notif, revenir aux icônes dans le widget". Tapping the peeked text used to open the original notification/app while leaving the peek showing (only a dismissal closed it, see below) — it now closes the peek and drops back to that view's tile grid at the same time it opens the notification. The tap's `PendingIntent` is no longer wired straight to the real target: it's wrapped in one of this app's own broadcasts (`NowBarWidgetProvider.openPeekContentPendingIntent`, carrying the real target as a `PendingIntent` extra) that closes the peek and rebuilds the widget first, then fires the real target itself.
- While peeking, the small toggle under the left column keeps showing the rotating-arrows glyph as a "back to the tile grid" button (Yann: "Mettre les flèches tournantes pour le symboliser"), still bound to the close action. **The big 32dp icon slot no longer also switches to the arrows (FIXED 18/09/2026)** — Yann: "vue texte dans widget : mettre l'icone de l'appli à gauche au lieu des grosses flèches qui tournent. Garder les petites." — it now shows the peeked entry's own source-app icon instead (Sofascore for a Sport match, the mirrored app's icon otherwise), same treatment the true Dernière notif view already gives it; the small arrows underneath are unchanged, and both stay bound to the same close action (same enlarged-tap-zone reasoning as the left toggle below).
- Dismissing the peeked notification, or its disappearing for any other reason (an action button that marked it read/archived it, the source app clearing it, a normal shade swipe), closes the peek automatically and drops back to the tile grid (Yann: "revenir automatiquement aux icônes") — `closePeekIfShowing` is called from both listener services' removal/dismiss paths, matched either by the Sofascore match key (Sport) or by the exact `(key, postTime)` pair when known, falling back to a key-prefix match for the rare case where it isn't (the pre-existing Dernière notif dismiss button never tracked a post time).
- If the peeked entry is simply gone by the time the widget next redraws (stale state after e.g. a process restart), it self-heals the same way rather than showing an empty peek: the peek is silently closed and the widget falls back to its normal 3-view rendering.

**Bell icon for "no specific app" contexts (NEW 18/09/2026)** — Yann: "Remplacer l'icône sur la gauche en vue toutes notifs par une cloche représentant notification. Idem quand il n'y a pas de notifs en vue texte." `ic_notification_bell` (a generic Material bell glyph, matching the hand-drawn-white-silhouette style already used by `ic_widget_dismiss`/`ic_widget_switch_view`) now replaces `ic_stat_mirror` as the left icon in the Toutes-notifs view and whenever the Dernière notif view has nothing to show.

**Left button, bigger tap zone (NEW 18/09/2026)** — Yann: "l'icône sur la gauche sert à changer la vue, laisser flèches en dessous mais élargir la zone à l'icône pour que ce soit plus facile à cliquer". The same toggle `PendingIntent` is now bound to both the small arrows glyph AND the 32dp app icon right above it, so the pair reads as one effectively bigger tap target with no visual change — same trick reused for the peek's own back button above.

**Right button, context-dependent icon (NEW 18/09/2026, reworked same day)** — Yann: "En vue sport, sur la droite, remplacer les flèches par un bouton cloche et flèches qui tourne [...] En vue toutes notifs, mettre l'icone blanche de Sofascore avec flèches aussi". Same toggle action as before (Sport ↔ Toutes notifs); the icon itself was reworked again later the same day (Yann: "sur la droite, supprimer les flèches et agrandir l'icône cloche et ballon. Remplacer le ballon par une icône de terrain de foot stylisee") — no more arrows, no more Sofascore-derived silhouette, and the button grew from a 20dp box (a ~14dp visible glyph) to 28dp (~22dp visible):
- On **Sport**, the plain bell glyph (`ic_notification_bell`) alone, hinting at the Toutes-notifs view it leads to.
- On **Toutes notifs**, the new stylized football-pitch glyph (`ic_football_pitch`, see below), hinting at the Sport view it leads to. The runtime-composited "arrows badged with a white silhouette of Sofascore's own icon" this used to be (`sofascoreToggleIconBitmap`/`whiteSilhouette`) is gone along with `ic_widget_switch_to_notifs.xml` — both were removed once the icon no longer needed either the arrows or a derived bitmap.

**Football-pitch icon, and used bigger on the left too (NEW 18/09/2026)** — Yann: "Remplacer le ballon par une icône de terrain de foot stylisee : tour blanc, ligne du milieu, surface de réparation et rond central. Lignes blanches pour tout ça. Afficher le même signe en plus grand sur la gauche en vue sport." `ic_football_pitch` is a new vector drawable — white outline only, transparent background, matching this app's other hand-drawn widget glyphs — with exactly the four elements asked for: the outer touchline border, the halfway line, a penalty area at each end, and the center circle. It's drawn in **portrait** orientation, i.e. the reference pitch photo Yann attached (a landscape pitch, goals on the left/right) rotated 90° so the halfway line runs horizontally and a penalty area sits at the top and at the bottom instead of the sides (Yann: "comme sur l'image jointe [...] mais en la tournant pour que les cages du gardien soient en haut et en bas"). Besides the right toggle above, it also replaces Sofascore's own app icon as `widget_app_icon` in the Sport view (`NowBarWidgetProvider.applySofascoreIcon`) — shown there at the left column's full 32dp size, i.e. bigger than the right toggle's copy, exactly as asked ("en plus grand sur la gauche").
...
33. Confirm Sofascore still appears normally in the Accueil app-selection list and still works exactly as before there (real Now Bar notification, Dernière notif view), then confirm a live Sofascore match's tile in Toutes notifs always renders as a match card (score/period), never as a generic image+title tile, and never as two separate tiles for the same match.
